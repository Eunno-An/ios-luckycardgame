...
final class LuckyCardGameTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        do {
            try testAppropriateSplit(attendeeNum: .three)
            try testPlayerCardIsSortedAscending(attendee: luckyCardGame.getMyInfo())
            try testPlayerCardIsSortedDescending(attendee: luckyCardGame.getMyInfo())
            try testBottomCardIsSortedAscending(bottomLuckyCards: luckyCardGame.getBelowLuckyCards())
            try testBottomCardIsSortedDescending(bottomLuckyCards: luckyCardGame.getBelowLuckyCards())
            //try testPlayerDeckHasThreeDuplicatedNum(attendee: luckyCardGame.getMyInfo())
    ...
    func testPlayerCardIsSortedAscending(attendee: Attendee) throws{
        let luckyCards: [LuckyCard] = attendee.getDeck().getCards()
    ...
    func testPlayerCardIsSortedDescending(attendee: Attendee) throws{
        let luckyCards: [LuckyCard] = attendee.getDeck().getCards()
        let sortedLuckyCards: [LuckyCard] = luckyCards.sorted{
            $0.number.rawValue > $1.number.rawValue
        }
    ...
    func testPlayerDeckHasThreeDuplicatedNum(attendee: Attendee) throws{
        var numCountArr: [Int] = Array(repeating: 0, count: 12)
        var isDeckHasThreeDupNum: Bool = false
        for card in attendee.getDeck().getCards(){
        ...
        XCTAssertFalse(isDeckHasThreeDupNum, "같은 숫자 카드 3장을 가져버렸군요.")
    }
    ...
    func testSameThreeNumber_MinMaxVal_of_Deck_and_AnyCardInBottomDeck(attendee: Attendee, bottomCards: [LuckyCard]) throws{
        var sortedLuckyCards: [LuckyCard] = attendee.getDeck().getCards().sorted{
            $0.number.rawValue > $1.number.rawValue
        }
        var isMinMaxValSame: Bool = (sortedLuckyCards.first?.number == sortedLuckyCards.last?.number)
        
        XCTAssertTrue(isMinMaxValSame, "플레이어 덱의 가장 낮은 숫자와 가장 높은 숫자가 서로 다릅니다!")
        
        var is_BottomCards_contains_MinMaxVal_of_LuckyCardArray: Bool = bottomCards.map{$0.number.rawValue}.contains(sortedLuckyCards[0].number.rawValue)
